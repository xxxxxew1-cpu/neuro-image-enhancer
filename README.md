...
> Демо (GitHub Pages): _добавьте ссылку после первого деплоя_ →
...
Поставляемая модель обучена 800 шагов; на отложенной выборке коррекция снижает
MSE к эталону на **~91 %** (0.0234 → 0.0022). Подробности — в
[training/README.md](training/README.md). Модель уже включена в репозиторий
(`public/models/enhance/`), переобучать не обязательно.
